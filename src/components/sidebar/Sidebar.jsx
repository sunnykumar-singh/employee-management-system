...
const Sidebar = ({ isOpen, onClose }) => (
  <>
    ...
    <aside
      className={`fixed inset-y-0 left-0 z-50 flex w-60 flex-col bg-[#03142d] px-3 py-5 shadow-2xl shadow-slate-950/30 transition-transform duration-300 lg:translate-x-0 ${
    ...
    >
      ...
      <nav className="min-h-0 flex-1 space-y-1 overflow-y-auto border-t border-white/15 pt-4 pr-1">
        {navigationItems.map((item) => <SidebarItem key={item.to} {...item} />)}
      </nav>
