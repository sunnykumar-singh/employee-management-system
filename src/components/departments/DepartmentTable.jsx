...
const DepartmentTable = ({departments, onView, onEdit, onDelete,}) => (
  <section className="overflow-hidden rounded-b-xl border-x border-b border-[#e4eaf2] bg-white shadow-[0_2px_8px_rgba(16,24,40,0.02)]">
    <div className="overflow-x-auto">
      {departments.length ? <table className="w-full min-w-[1050px] border-collapse"><DepartmentTableHeader /><tbody>{departments.map((department) => <DepartmentRow  department={department} onView={onView} onEdit={onEdit}onDelete={onDelete} />)}</tbody></table> : <EmptyState />}
    </div>
